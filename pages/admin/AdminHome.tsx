      ...
      <div className="p-5 space-y-6">
        <section>
          <div className="flex gap-4 overflow-x-auto no-scrollbar snap-x -mx-5 px-5">
             <StatCard title="Dues Collected" value="₹4.5L" trend="+12%" icon="payments" color="text-purple-600 bg-purple-50" onClick={() => setView('finance')} />
             <StatCard title="Active Members" value="482" trend="98%" icon="group" color="text-blue-600 bg-blue-50" onClick={() => setView('members')} />
             <StatCard title="Complaints" value="12" trend="-4" icon="report_problem" color="text-orange-600 bg-orange-50" onClick={() => setView('complaints')} />
          </div>
        </section>
        ...
        <section>
          <div className="flex justify-between items-center mb-4"><h2 className="text-lg font-black text-slate-900 tracking-tight">Quick Controls</h2></div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <QuickActionButton label="Complaints" icon="report_problem" color="text-orange-600 bg-orange-50" onClick={() => setView('complaints')} />
...
const StatCard = ({ title, value, trend, icon, color, onClick }: any) => (
  <div onClick={onClick} className="min-w-[200px] snap-center rounded-[2rem] p-6 bg-white shadow-sm border border-slate-100 flex flex-col justify-between h-40 active:scale-95 transition-all cursor-pointer">
    <div className={`size-11 rounded-2xl flex items-center justify-center shrink-0 ${color}`}><span className="material-symbols-outlined text-xl">{icon}</span></div>
    <div>
      <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-1">{title}</p>
      <div className="flex justify-between items-end"><h4 className="text-2xl font-black text-slate-900">{value}</h4><span className="text-[10px] font-black text-success">{trend}</span></div>
    </div>
  </div>
);

const QuickActionButton = ({ label, icon, color, onClick }: any) => (
  <button onClick={onClick} className="flex flex-col items-center justify-center gap-3 p-6 rounded-[2rem] bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all active:scale-95 group">
